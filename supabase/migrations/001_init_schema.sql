...
-- 5. PRODUCT_PRICING (Admin-only access)
...
CREATE POLICY "stock_movements_employee_insert" ON stock_movements
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'funcionario'
    )
  );
...
AS $$
  SELECT
    COALESCE(SUM(pp.sale_price * sm.quantity_change), 0) AS faturamento_bruto,
    COALESCE(
      SUM((pp.sale_price - pp.cost_price) * sm.quantity_change),
      0
